...
const makeMiddleEarth = () => {
  ...
  const $ME = $("<section>").attr("id", "middle-earth");
...
for(let i = 0; i < hobbits.length; i++){
  const $hobbits = $("<li>").addClass("hobbit").text(`${hobbits[i]}`);
  // 3. also, give each hobbit (`li`) a class of "hobbit"
$hobbitUl.append($hobbits);
}
...
for(let i = 0; i < buddies.length; i++){
  const $bud = $("<li>").text(`${buddies[i]}`).addClass("buddy");
  $daBuds.append($bud);
}
...
const $nodes = [$("#The-Shire").children("ul")]
$nodes[0].appendTo("#Rivendell");
};
...
const $text = $("<h1>").text("The Fellowship");
$fellowship.append($text);
...
const blowHorn = alert("The horn of Gondor has been blown!");
...
const $fro = $(".hobbit").eq(0).remove();
const $sam = $(".hobbit").eq(0).remove();
$sam.appendTo("#Mordor");
$fro.appendTo("#Mordor");

  // 2. add a div with an id of "mount-doom" to Mordor
const $mntDoom = $("<div>").attr("id", "mount-doom");
...
const $ring = $("#Mordor").children("li").eq(1).children("div").remove();
$div.append($ring);
