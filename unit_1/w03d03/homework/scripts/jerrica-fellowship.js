...
// TIP! Make a console.log for each of your functions to make sure that, when you click, the correct function is being called!
// e.g. for chapter one, console.log("Trying to make middle earth.");
...
const makeMiddleEarth = () => {
  ...
  for (const land of lands) {
    ...
    const $landArticle = $('<article>').attr('id', land)
...
const makeHobbits = () => {
  ...
  // 2. make each hobbit an li element and append it to the 'ul' you just created
    ...
    for (const hobbit of hobbits) {
      const $hobbitLi = $('<li>').addClass('hobbit').text(`${hobbit}`).appendTo($hobbitsUl)
    }
  // 4. append the ul to the shire
    // hint: get 'The-Shire' by using its id
    $('#The-Shire').append($hobbitsUl)
};
...
const makeBaddies = () => {
  ...
  for (const baddy of baddies) {
    const $baddyLi = $('<li>').addClass('baddy').text(`${baddy}`).appendTo($baddiesUl)
  }
...
const makeBuddies = () => {
  ...
  for (const buddy of buddies) {
    const $buddyLi = $('<li>').addClass('buddy').text(`${buddy}`).appendTo($buddiesUl)
  }
...
const leaveTheShire = () => {
  // 1. grab the hobbits (the ul in which they reside) and move them to Rivendell
    ...
    const $hobbitsUl = $('#The-Shire').children().eq(1)
...
const forgeTheFellowShip = () => {
  ...
  $theFellowship.append($('.hobbit').parent())
  $theFellowship.append($('.buddy').parent())
};
...
// =============================================
// DO NOT CHANGE ANYTHING UNDERNEATH THIS LINE!
// =============================================
// =====================================
