...
`cd` into the `starter-code` directory or revisit the calculator you made for homework last night.

Make sure you have an `index.html` `app.js` and `main.css` and get jQuery wired up. Remember, you can check if jQuery is properly connected with `console.log($)`.

If you're using the `starter-code` directory, examine your files and get yourself oriented.
